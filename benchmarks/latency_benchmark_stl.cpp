        ...
        for (size_t size = min_message_size; size <= max_message_size; size *= 2)
        {
            ...
            for (size_t iteration = 0; iteration < nb_iterations; iteration++)
            {
                if (comm.get_rank() == 0)
                {
                    ...
                    comm.send(mppi::Destination(1), mppi::Tag(1), send_buf);
                    comm.recv(mppi::Source(1), mppi::Tag(1), recv_buf);
                ...
                {
                    comm.recv(mppi::Source(0), mppi::Tag(1), recv_buf);
                    comm.send(mppi::Destination(0), mppi::Tag(1), send_buf);
                }
            }
